...
# Locate local data folder (data/blob/ relative to repo root)
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"
LOCAL_DATA_DIR="$REPO_ROOT/data/blob"

# Container name: use env var, or detect from the single subfolder inside data/blob/
CONTAINER_NAME="${AZURE_BLOB_CONTAINER_NAME:-}"
if [ -z "$CONTAINER_NAME" ] && [ -d "$LOCAL_DATA_DIR" ]; then
    # Auto-detect: use the first (and typically only) subfolder name
    CONTAINER_NAME=$(ls -1 "$LOCAL_DATA_DIR" | head -n1)
fi
CONTAINER_NAME="${CONTAINER_NAME:-foundry-iq-data}"

LOCAL_UPLOAD_SRC="$LOCAL_DATA_DIR/$CONTAINER_NAME"

echo ""
echo "[2/7] Locating local sample data..."

if [ ! -d "$LOCAL_UPLOAD_SRC" ]; then
    echo "✗ Local data directory not found: $LOCAL_UPLOAD_SRC"
    echo "  Expected folder tree under data/blob/<container-name>/"
    exit 1
fi

FILE_COUNT=$(find "$LOCAL_UPLOAD_SRC" -type f | wc -l | tr -d ' ')
echo "✓ Found $FILE_COUNT files in $LOCAL_UPLOAD_SRC"

# Step 3: Upload local folder tree to Azure Blob Storage
echo ""
echo "[3/7] Uploading local data tree to Azure Blob Storage..."
...
az storage blob upload-batch \
    ...
    --source "$LOCAL_UPLOAD_SRC" \
    --auth-mode login \
    --overwrite \
    --output none

echo "✓ Uploaded $FILE_COUNT files to container: $CONTAINER_NAME"
...
echo "  1. Blob Storage: $CONTAINER_NAME container with $FILE_COUNT files"
