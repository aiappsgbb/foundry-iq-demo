...
LOCAL_UPLOAD_SRC="$LOCAL_DATA_DIR/$STORAGE_CONTAINER_NAME"

# Step 1: Locate local sample data
echo "[1/4] Locating local sample data..."

if [ ! -d "$LOCAL_UPLOAD_SRC" ]; then
    echo "✗ Local data directory not found: $LOCAL_UPLOAD_SRC"
    echo "  Expected folder tree under data/blob/$STORAGE_CONTAINER_NAME/"
    exit 1
fi
...
# Step 2: Upload local folder tree to Azure Blob Storage
echo ""
echo "[2/4] Uploading local data tree to Azure Blob Storage..."
...
echo "✓ Uploaded $FILE_COUNT files to container: $STORAGE_CONTAINER_NAME"
...
echo "  1. Blob Storage: $STORAGE_CONTAINER_NAME container with $FILE_COUNT files"
