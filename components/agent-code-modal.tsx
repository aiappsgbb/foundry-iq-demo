      ...
      "mcp": [${selectedKnowledgeBases.map(baseName => `
        {
          ...
          "headers": {
            "Authorization": "Bearer $AZURE_SEARCH_BEARER_TOKEN"
          }
        }`).join(',')}
      ]
    }
...
mcp_resources = [${selectedKnowledgeBases.map(baseName => `
    {
        ...
        "require_approval": "never",
        "headers": {
            "Authorization": f"Bearer {os.getenv('AZURE_SEARCH_BEARER_TOKEN')}"
        }
    }`).join(',')}
]
              ...
              <div className="space-y-8">
                <div className="bg-bg-info-subtle border border-stroke-info rounded-lg p-4">
                  <p className="text-sm text-fg-info">
                    <strong>Prerequisites:</strong> Set environment variable <code>FOUNDRY_BEARER_TOKEN</code>. For Search access, use <code>az account get-access-token --resource https://search.azure.com</code>
                  </p>
                </div>
